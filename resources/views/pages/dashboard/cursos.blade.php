...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      <div
        ...
        <div class="flex items-center justify-between">
          ...
          <div
            class="w-12 h-12 bg-blue-100 rounded-xl flex items-center justify-center group-hover:scale-101 transition-transform duration-200">
      ...
      <div
        ...
        <div class="flex items-center justify-between">
          ...
          <div
            class="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center group-hover:scale-101 transition-transform duration-200">
      ...
      <div
        ...
        <div class="flex items-center justify-between">
          ...
          <div
            class="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center group-hover:scale-101 transition-transform duration-200">
      ...
      <div
        ...
        <div class="flex items-center justify-between">
          ...
          <div
            class="w-12 h-12 bg-yellow-100 rounded-xl flex items-center justify-center group-hover:scale-101 transition-transform duration-200">
    ...
    <div id="coursesGrid" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      @foreach ($cursos as $index => $curso)
        <div
          ...
          data-title="{{ strtolower($curso->title) }}" data-description="{{ strtolower($curso->description) }}"
          ...
          <div class="p-6 border-b border-gray-100">
            <div class="flex items-start justify-between">
              <div class="flex-1">
                ...
                <p class="text-gray-600 text-sm mb-3 line-clamp-2">{{ $curso->description }}</p>
              ...
              <div class="flex items-center space-x-2 ml-4">
                ...
                <button onclick="confirmDeleteCourse({{ $curso->id }}, '{{ $curso->title }}')"
                ...
                @foreach ($curso->files as $archivo)
                  <div
                    ...
                    <div class="flex items-center space-x-3">
                      <div
                        class="w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-xs
                                {{ $archivo->type === 'pdf'
                                    ? 'bg-red-500'
                                    : ($archivo->type === 'doc' || $archivo->type === 'docx'
                                        ? 'bg-blue-500'
                                        : ($archivo->type === 'image'
                                            ? 'bg-green-500'
                                            : 'bg-gray-500')) }}">
                        @if ($archivo->type === 'pdf')
                          PDF
                        @elseif(in_array($archivo->type, ['doc', 'docx']))
                          DOC
                        @elseif($archivo->type === 'image')
                          IMG
                        @else
                          {{ strtoupper(substr($archivo->type, 0, 3)) }}
                        @endif
                      </div>
                      <div>
                        <p class="text-sm font-medium text-gray-900">{{ $archivo->title }}</p>
                        <p class="text-xs text-gray-500">{{ $archivo->original_name }}</p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-2">
                      ...
                      <button onclick="confirmDeleteFile({{ $archivo->id }}, '{{ $archivo->title }}')"
  ...
  <div id="courseModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto animate-scale-in">
      ...
      <div class="p-8">
        <form id="courseForm" method="POST" action="{{ route('dashboard.cursos.store') }}"
          ...
          <div id="initialFileSection" class="space-y-4 border-t border-gray-200 pt-6">
            ...
            <div class="space-y-2">
              <label for="file_title" class="block text-sm font-semibold text-gray-700">
                Título del archivo *
              </label>
      ...
      <div class="p-8">
        <form id="fileForm" method="POST" enctype="multipart/form-data" class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <input type="file" id="file_archivo" name="file" required
          ...
          <div class="flex justify-end space-x-4 pt-6 border-t border-gray-100">
            ...
            <button type="submit"
              class="px-8 py-3 bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white font-semibold rounded-xl transition-all duration-200 transform hover:scale-101">
              Subir Archivo
            </button>
          </div>
        </form>
  ...
  <script>
    ...
    function openCourseModal() {
      ...
      document.getElementById('file_title').required = true;
    ...
    function openFileModal(courseId) {
      ...
      form.action = `/dashboard/cursos/${courseId}/archivos`;
    ...
    function confirmDeleteCourse(courseId, courseName) {
      ...
      courseNameElement.textContent =
        `¿Estás seguro de que quieres eliminar "${courseName}"? Esta acción eliminará también todos sus archivos.`;
